...
import {
    ...
    HamburguerMenu, Brand, MenuButton, Sidebar,
    SidebarLabel, Artist, ArtistProfile, SidebarRuler,
    SidebarContainer
} from './styled'
    ...
    return <Header>
        ...
        <Sidebar isOpened={isSidebarOpened}>
            <SidebarContainer>
                ...
                { artists.communities.filter(artist => artist.joined).map(artist => {
                    return (
                        <Artist key={artist.name}>
                            <ArtistProfile src={artist.iconImgPath} width={30} height={30} />
                            {artist.name}
                        </Artist>
                    )
                }) }
                ...
                { artists.communities.sort((a, b) => a.id < b.id).filter(artist => !artist.joined).map(artist => {
                    return (
                        <Artist key={artist.name}>
                            <ArtistProfile src={artist.iconImgPath} width={30} height={30} />
                            {artist.name}
